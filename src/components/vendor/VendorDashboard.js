...
const VendorDashboard = ({ user = defaultUser, onLogout }) => {
  ...
  // User profile state - Initialize with empty values
  const [userProfile, setUserProfile] = useState({
    fullName: '',
    email: '',
    phone: '',
  ...
  useEffect(() => {
    ...
    // Only set user profile if user data is provided, otherwise keep empty
  ...
  // Prescription Verification Functions - UPDATED
  const approvePrescription = (prescriptionId) => {
    const prescription = prescriptions.find(p => p.id === prescriptionId);
    if (prescription) {
      ...
      // Find the corresponding order and move it to ready
      const order = orders.pending.find(o => o.id === prescription.orderId);
      if (order) {
        setOrders(prev => ({
          ...prev,
          pending: prev.pending.filter(o => o.id !== prescription.orderId),
          ready: [...prev.ready, { ...order, approvedTime: new Date().toLocaleString() }]
        }));
      }
      
      setSelectedPrescription(null);
      showNotification('Prescription Approved', `Prescription for order ${prescription.orderId} has been approved`);
    }
  };
  ...
  const rejectPrescription = (prescriptionId) => {
    ...
    if (prescription) {
      ...
      // Find the corresponding order and move it to cancelled
      const order = orders.pending.find(o => o.id === prescription.orderId);
      if (order) {
        setOrders(prev => ({
          ...prev,
          pending: prev.pending.filter(o => o.id !== prescription.orderId),
          cancelled: [...prev.cancelled, { ...order, cancelledTime: new Date().toLocaleString(), cancellationReason: 'Prescription rejected' }]
        }));
      }
      
      setSelectedPrescription(null);
      showNotification('Prescription Rejected', `Prescription for order ${prescription.orderId} has been rejected`);
    }
  };
