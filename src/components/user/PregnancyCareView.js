...
// Use the SAME subscription plans as UserDashboard
const PREGNANCY_SUBSCRIPTION_PLANS = [
  {
    id: 'pregnancy-monthly',
    title: 'Pregnancy Care Monthly',
    price: 699,
    duration: 'month',
    savings: 'Save 15%',
    popular: false,
    features: [
      'Complete pregnancy diet plan (9 months)',
      'Monthly medication guidance',
      'Daily exercises & yoga routines',
  ...
  },
  {
    ...
    features: [
      ...
      'Baby care essentials kit',
      'Nutritionist consultations',
      'Yoga instructor sessions',
...
const PregnancyCareView = ({ 
  ...
  // Get active pregnancy subscription from UserDashboard
  const activePregnancySubscription = userSubscriptions?.find(sub => 
    sub.planType === 'pregnancyCare' && sub.status === 'active'
  );

  // Check if user has premium access
  const hasPremiumAccess = () => {
    return isSubscribed && activePregnancySubscription;
  };
  ...
  // Handle upgrade in PregnancyCareView
  const handleUpgradeInPregnancy = (plan) => {
    if (!activePregnancySubscription) {
      setShowSubscriptionModal(true);
      return;
    }
    ...
    if (handleUpgradeSubscription) {
      handleUpgradeSubscription({
        ...
        subscription: activePregnancySubscription,
        annualPlan: plan
      });
    } else {
  ...
  // Subscription Plans Section in Pregnancy Care
  const SubscriptionPlansSection = () => (
    <div className="subscription-plans-section">
      <h3><Icon name="shield" size={24} /> Pregnancy Care Subscription Plans</h3>
      <p>Choose the perfect plan for your pregnancy journey</p>
      
      <div className="subscription-cards-grid">
        {PREGNANCY_SUBSCRIPTION_PLANS.map(plan => {
          const isActive = activePregnancySubscription?.planId === plan.id;
          const isMonthlyActive = activePregnancySubscription?.duration === 'month';
          const canUpgrade = isMonthlyActive && plan.duration === 'year';
          
          return (
            <div
              key={plan.id}
              className={`subscription-card ${isActive ? 'active' : ''} ${plan.popular ? 'popular' : ''}`}
            >
              ...
              <div className="subscription-header">
                ...
                <div className="subscription-price">
                  ...
                  <span className="duration">per {plan.duration}</span>
                </div>
              </div>
              ...
              {isActive ? (
                ...
                <button
                  ...
                  onClick={() => handleUpgradeInPregnancy(plan)}
                ...
                <button
                  ...
                  onClick={() => handleSubscribeInPregnancy(plan)}
                  disabled={paymentLoading}
                >
  ...
  return (
    <div className="pregnancy-container">
      ...
      <div className="subscription-status-bar">
        {activePregnancySubscription ? (
          <div className={`subscription-status active`}>
            <div>
              <strong>{activePregnancySubscription.title}</strong>
              <p>
                Active until {new Date(activePregnancySubscription.endDate).toLocaleDateString()} • 
                {activePregnancySubscription.duration === 'year' ? ' Annual Plan' : ' Monthly Plan'}
              </p>
            </div>
            <div className="subscription-actions">
              {activePregnancySubscription.duration === 'month' && (
                <button 
                  className="upgrade-btn" 
                  onClick={() => handleUpgradeInPregnancy(
                    PREGNANCY_SUBSCRIPTION_PLANS.find(p => p.id.includes('annual'))
                  )}
                  disabled={paymentLoading}
                >
                  <Icon name="arrow" size={16} /> {paymentLoading ? 'Processing...' : 'Upgrade to Annual'}
                </button>
              )}
          ...
          <div className="subscription-status inactive">
            <p>No active subscription. <a href="#plans" onClick={(e) => { e.preventDefault(); setActiveTab('plans'); }}>Subscribe now</a> for premium features.</p>
          </div>
        )}
      </div>
      ...
      <div className="dashboard-section">
        <div className="tabs-container">
          ...
          {/* Tab Content - REMOVED SCROLLBARS */}
                ...
                <div className="section-spacing">
                  ...
                  <div className="grid">
                    <div className="feature-card">
                      <div className="feature-header"><Icon name="stethoscope" size={24} /><h4>First Consultation Free</h4></div>
                      <p>Visit hospital for first consultation, then continue with online checkups</p>
                    </div>
                    <div className="feature-card">
                      <div className="feature-header"><Icon name="ai" size={24} /><h4>AI-Supported Diet Plan</h4></div>
                      <p>Personalized nutrition plan based on your health data</p>
                    </div>
                    <div className="feature-card">
                      <div className="feature-header"><Icon name="home" size={24} /><h4>Home Health Checkups</h4></div>
                      <p>Medical staff visits for regular health monitoring</p>
                    </div>
                    <div className="feature-card">
                      <div className="feature-header"><Icon name="security" size={24} /><h4>Secure File Locker</h4></div>
                      <p>End-to-end encrypted storage for all medical reports</p>
                    </div>
                  </div>
                </div>
                    ...
                    {medicalReports.map(report => (
                      <div key={report.id} className="file-card">
                        <div className="file-header">
                          <div className="file-icon"><Icon name={report.type === 'ultrasound' ? 'camera' : 'file'} size={24} /></div>
                  ...
                  <div className="card card-spacing premium-locked">
                    <div className="premium-locked-content">
                      ...
                      <button className="button" onClick={() => {
                        ...
                        <Icon name="shield" size={20} /> Unlock with Subscription
                      </button>
                    </div>
                  </div>
                )}
        ...
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '450px' }}>
            ...
            <div className="modal-body text-center">
              ...
              <h4>Subscription Payment Cancelled</h4>
                ...
                <div className="flex gap-15">
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '14px', marginTop: '5px' }}>
                      The payment process was interrupted or cancelled. Your subscription was not activated.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="modal-footer gap-10">
              ...
              >
                <Icon name="arrowRight" size={18} /> View Subscription Plans
              </button>
              <button 
                className="secondary-button" 
                onClick={() => setShowPaymentCancelledModal(false)}
