...
const BloodBankView = ({ setActiveView, addNotification }) => {
  ...
  const FindBloodTab = () => (
    <div style={styles.contentSection}>
      <h3 style={{ marginBottom: '20px', color: '#333' }}>🔍 Find Blood</h3>
      ...
      <div>
        <h4 style={{ marginBottom: '15px', color: '#333' }}>Active Blood Requests</h4>
        {requests.filter(req => req.status === 'Active').map(request => (
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div>
                <div style={{ fontWeight: 'bold', color: '#333' }}>{request.patientName}</div>
  ...
  const DonateTab = () => (
    <div style={styles.contentSection}>
      ...
      <h3 style={{ marginBottom: '20px', color: '#333' }}>🏥 Nearby Blood Banks</h3>
      {bloodBanks.map(bank => (
              ...
              {bank.availableGroups.map(group => (
                <span 
                  ...
                  style={{
                    ...
                    backgroundColor: group === userProfile.bloodGroup ? '#ffebee' : '#f8f9fa',
                    color: group === userProfile.bloodGroup ? '#d32f2f' : '#666'
                  }}
                >
                  {group}
                </span>
  ...
  const HistoryTab = () => (
    <div style={styles.contentSection}>
      <h3 style={{ marginBottom: '20px', color: '#333' }}>📋 Donation History</h3>
      {donationHistory.map(donation => (
        <div key={donation.id} style={styles.donorCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div>
              <div style={{ fontWeight: 'bold', color: '#333' }}>Donation #{donation.id}</div>
              <div style={{ fontSize: '0.9rem', color: '#666' }}>{donation.venue}</div>
  ...
  const CompatibilityTab = () => (
    <div style={styles.contentSection}>
      <h3 style={{ marginBottom: '20px', color: '#333' }}>🩸 Blood Group Compatibility</h3>
        ...
        {Object.entries(bloodCompatibility).map(([group, compatible]) => (
          <div key={group} style={styles.donorCard}>
            <div style={{ 
              fontSize: '1.5rem', 
              fontWeight: 'bold', 
              color: group === userProfile.bloodGroup ? '#d32f2f' : '#7C2A62',
              marginBottom: '10px'
            }}>
              {group}
            </div>
              ...
              {compatible.map(bg => (
                <span 
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    fontSize: '0.7rem',
                    backgroundColor: bg === userProfile.bloodGroup ? '#ffebee' : '#f8f9fa',
                    color: bg === userProfile.bloodGroup ? '#d32f2f' : '#666'
                  }}
                >
                  {bg}
                </span>
