...
const AppointmentsView = ({
  ...
  cancelAppointment,
  viewAppointmentDetails,
  sendToDoctor // New prop for sending schedule to doctor
}) => {
  const BackButton = ({ onClick, text = 'Back' }) => (
    <button 
      style={styles.backButton}
  ...
  // Enhanced reschedule function with one-time limitation
  const handleReschedule = (appointment) => {
    if (appointment.rescheduled) {
      alert('This appointment has already been rescheduled once. Please contact support for further changes.');
      return;
    }
    ...
    if (newDate && newTime) {
      rescheduleAppointment(appointment.id, newDate, newTime, true); // Pass true to mark as rescheduled
    }
  };

  // Send schedule to doctor
  const handleSendToDoctor = (appointment) => {
    if (sendToDoctor) {
      sendToDoctor(appointment);
    } else {
      // Default implementation
      alert(`Schedule sent to Dr. ${appointment.doctorName} successfully!`);
    }
  };
              ...
              <div style={styles.appointmentHeader}>
                <div>
                  ...
                  {appointment.rescheduled && (
                    <p style={styles.rescheduledBadge}>Rescheduled</p>
                  )}
                </div>
                <div style={styles.appointmentStatus}>
                  <span style={{
              ...
              <div style={styles.appointmentActions}>
                {appointment.status === 'Scheduled' && (
                  <>
                    ...
                    >
                      {appointment.rescheduled ? 'Already Rescheduled' : 'Reschedule'}
                    </button>
                    <button 
                      style={styles.sendToDoctorButton}
