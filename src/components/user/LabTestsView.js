...
const LabTestsView = ({ setActiveView, addNotification }) => {
        ...
        {filteredTests.map(test => (
          <div key={test.id} style={styles.testCard}>
            ...
            <button 
              style={{
                ...styles.button,
                backgroundColor: cart.some(item => item.id === test.id) ? '#28a745' : '#7C2A62'
              }}
      ...
      {cart.length === 0 ? (
        ...
        <div>
            ...
            <div key={test.id} style={styles.bookingCard}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.9rem', color: '#666' }}>Report: {test.reportTime}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                  <div style={{ fontWeight: 'bold', color: '#7C2A62' }}>₹{test.price}</div>
                  <button 
          ...
          <div style={{ 
            ...
            <div>
              <div style={{ fontWeight: 'bold', color: '#333' }}>Total Amount</div>
              <div style={{ fontSize: '0.9rem', color: '#666' }}>{cart.length} tests selected</div>
            </div>
            <div style={{ fontWeight: 'bold', color: '#7C2A62', fontSize: '1.2rem' }}>
              ₹{getTotalPrice()}
            </div>
          </div>
          <button 
            style={{ ...styles.button, marginTop: '20px', fontSize: '1rem', padding: '15px' }}
            onClick={bookTests}
          >
            🧪 Book Tests Now
          </button>
      ...
      {bookingHistory.map(booking => (
        <div key={booking.id} style={styles.bookingCard}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
            <div>
              <div style={{ fontWeight: 'bold', color: '#333', fontSize: '1.1rem' }}>Booking #{booking.id}</div>
            ...
            <div style={{
              ...
              fontSize: '0.8rem',
              fontWeight: 'bold',
              backgroundColor: booking.status === 'Completed' ? '#e7f6e7' : '#fff3e0',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ fontWeight: 'bold', color: '#7C2A62' }}>₹{booking.amount}</div>
            {booking.results === 'Available' ? (
              ...
              <div style={{ fontSize: '0.8rem', color: '#666' }}>Results: {booking.results}</div>
            )}
          </div>
        </div>
      ))}
  ...
  const LabsTab = () => (
    <div style={styles.contentSection}>
      ...
      <div style={styles.testsGrid}>
        {labs.map(lab => (
          <div key={lab.id} style={styles.testCard}>
      ...
      <div style={styles.headerRow}>
        ...
        <div style={styles.cartButtonContainer}>
          <button 
            style={{
              ...styles.tab,
              backgroundColor: cart.length > 0 ? '#7C2A62' : '#f8f9fa',
              color: cart.length > 0 ? 'white' : '#666'
            }}
